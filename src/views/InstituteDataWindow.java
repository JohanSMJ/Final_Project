...
public class InstituteDataWindow extends JDialog{
	...
	public InstituteDataWindow( LabelsGUI[] headers,Object[][]matrix, ControllerApps controllerApps, boolean isAdmin,Institute institute) {
		this.setSize(1200, 700);
		this.setLocationRelativeTo(null);
		this.setDefaultCloseOperation(HIDE_ON_CLOSE);
		this.setResizable(false);
		this.setLayout(new BorderLayout());
		this.inItComponents(headers,matrix,controllerApps,isAdmin,institute);
		this.setVisible(true);
	}
	private void inItComponents( LabelsGUI[] headers,Object[][]matrix,ControllerApps controllerApps,boolean isAdmin,Institute institute) {
		this.inItMenu();
		this.inItForm(headers,matrix,institute);
		if (isAdmin) {
			this.setAdminCommands(controllerApps);
		}else {
			this.setCommands(controllerApps);
		}
		this.chanceLanguage(institute);
		String titlePrincipal=institute.getName().toUpperCase();
		this.setTitlePrincipal(titlePrincipal);
	}
	
	private void inItForm(LabelsGUI [] headers,Object[][]matrix ,Institute institute) {
	...
	public void chanceLanguage(Institute institute) {
		buttonPrint.setText(HandlerLanguage.languageProperties.getProperty(LabelsGUI.PRINT.name()));
		labelFundation.setText(HandlerLanguage.languageProperties.getProperty(LabelsGUI.FUNDATION_DATE.name())+": "+DateUtilities.calendarToString(institute.getDateFundation()));
	}
